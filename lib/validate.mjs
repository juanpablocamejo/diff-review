...
/** Schema for post-batch synthesis (intent + unified groups + id mapping). */
export const SYNTHESIS_SCHEMA = {
  type: "object",
  required: ["intent", "groups", "groupMap"],
  additionalProperties: true,
  properties: {
    intent: { type: "string", minLength: 40 },
    groups: REVIEW_SCHEMA.properties.groups,
    groupMap: { type: "object" },
  },
};
...

{
  ...
  "groups": [{ "id": "g1", "kind": "feat", "title": "feat(scope): summary ≤72 chars", "intent": "why this theme exists" }],
...
/** Synthesis turn after batched review: unify themes without resending diffs. */
export function synthesisContract() {
  return `OUTPUT: a single JSON object only. No markdown, no fences, no prose.
{
  "intent": "2-4 sentences for the whole branch",
  "groups": [{ "id": "g1", "kind": "feat", "title": "feat(scope): summary", "intent": "why" }],
  "groupMap": { "g11": "g1", "g21": "g1", "g12": "g2" }
}
groupMap maps every old group id from the partial review to a new unified group id.
groups must list every new id used in groupMap. 3-8 themes, semantic not per-folder.
TEXT LANGUAGE: Spanish for intent and groups.intent.`;
}
